<?php
class Car{
    function __construct($numberOfWheel, $power, $brand, $color, $flyingInstance=null){
        $this->numberOfWheel = $numberOfWheel;
        $this->power = $power;
        $this->brand = $brand;
        $this->color = $color;
        $this->flyingInstance = $flyingInstance;
    ...
    function endEngine(){
        // 若為父類別呼叫則flyingInstance會為null
        if (!is_null($this->flyingInstance)){
            $this->flyingInstance->foldWings();
        }
        echo "車子熄火\n";
    }
}
...
class flyingCar extends Car{
    ...
    function __construct($numberOfWheel, $power, $brand, $color){
        // (父類別的參數有什麼子類別也得要有) 
        parent::__construct($numberOfWheel, $power, $brand, $color, $this);
...
// $car1 = new Car(4,200,"BMW","銀色"); // 要設定flyingInstance的預設值
$car1->endEngine();
